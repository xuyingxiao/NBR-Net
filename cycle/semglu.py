...
from models.utils.mod import CMDTop,OpticalFlowEstimator, FeatureL2Norm, warp, \
    CorrelationVolume, deconv, conv, predict_flow, unnormalise_and_convert_mapping_to_flow
from models.utils.consensus_network_modules import MutualMatching, NeighConsensus, FeatureCorrelation
...
class SemanticGLUNet_model(nn.Module):
    ...
    def forward(self, im_target, im_source, im_target_256, im_source_256):
        ...
        with torch.no_grad():
            ...
            c15 = im1_pyr_256[-2]
            c25 = im2_pyr_256[-2]
            # print("im1_pyr[-2].shape:", im1_pyr[-2].shape) #torch.Size([1, 128, 64, 64])
            # print("im1_pyr[-1].shape:", im1_pyr[-1].shape) #torch.Size([1, 256, 32, 32])
            # print("im1_pyr_256[-4].shape:", im1_pyr_256[-4].shape) # torch.Size([1, 256, 32, 32])
            # print("im1_pyr_256[-3].shape:", im1_pyr_256[-3].shape) # torch.Size([1, 512, 16, 16])
            # print("im1_pyr_256[-2].shape:", im1_pyr_256[-2].shape) # torch.Size([1, 512, 8, 8])
        ...
        # print("c24_concat.shape:", c24_concat.shape)   #torch.Size([1, 1024, 16, 16])
        # print("c14_concat.shape:", c14_concat.shape)   #torch.Size([1, 1024, 16, 16])
        flow4 = self.coarsest_resolution_flow(c14_concat, c24_concat, h_256, w_256)
        up_flow4 = self.deconv4(flow4)
        # print("flow4.shape:", flow4.shape)   #torch.Size([1, 2, 16, 16])
        # print("up_flow4.shape:", up_flow4.shape)  # torch.Size([1, 2, 32, 32])
        ...
        # print("c23_concat.shape:", c23_concat.shape)   #torch.Size([1, 1280, 32, 32])
        # print("c13_concat.shape:", c13_concat.shape)   #torch.Size([1, 1280, 32, 32])

        warp3 = warp(c23_concat, up_flow_4_warping)
        # print("warp3.shape:", warp3.shape)  #torch.Size([1, 1280, 32, 32])
        ...
        # print("x.shape:", x.shape)  # torch.Size([1, 96, 32, 32])
        # print("flow3.shape:", flow3.shape)  # torch.Size([1, 2, 32, 32])
        ...
        # print("warp2.shape:", warp2.shape)  # torch.Size([1, 256, 32, 32])
        corr2 = correlation.FunctionCorrelation(tensorFirst=c12, tensorSecond=warp2)
        corr2 = self.leakyRELU(corr2)
        # print("corr2.shape:", corr2.shape)  # torch.Size([1, 81, 32, 32])
        ...
        # print("up_flow2.shape:", up_flow2.shape)  # torch.Size([1, 2, 64, 64])
        # print("up_feat2.shape:", up_feat2.shape)  # torch.Size([1, 2, 64, 64])


        # level 1/4 of original resolution
        ratio = 1.0 / 4.0
        warp1 = warp(c21, up_flow2*div*ratio)
        # print("warp1 shape",warp1.shape) #torch.Size([1, 128, 64, 64])
        corr1 = correlation.FunctionCorrelation(tensorFirst=c11, tensorSecond=warp1)
        corr1 = self.leakyRELU(corr1)
        # print("corr1.shape:", corr1.shape)  # torch.Size([1, 81, 64, 64])
        corr1 = torch.cat((corr1, up_flow2, up_feat2), 1)
        # print("corr1.shape:", corr1.shape)  # torch.Size([1, 85, 64, 64])
        x, res_flow1 = self.decoder1(corr1)
        # print("x.shape:", x.shape)  # torch.Size([1, 533, 64, 64])
        # print("res_flow1.shape:", res_flow1.shape)  # torch.Size([1, 2, 64, 64])
        flow1 = res_flow1 + up_flow2
        # print("flow1.shape:", flow1.shape)  # torch.Size([1, 2, 64, 64])
        ...
        # print("x.shape:", x.shape)  # torch.Size([1, 96, 64, 64])
        # print("flow1.shape:", flow1.shape)  # torch.Size([1, 2, 64, 64])
        if self.evaluation:
            return flow1
        else:
