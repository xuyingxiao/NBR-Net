...
class VGGPyramid(nn.Module):
    ...
    def forward(self, x, quarter_resolution_only=False, eigth_resolution=False):
        ...
        if quarter_resolution_only:
            ...
            #print(x_full.shape) #torch.Size([1, 64, 256, 256])
            #print(x_half.shape) #torch.Size([1, 64, 128, 128])
            #print(x_quarter.shape) #torch.Size([1, 128, 64, 64])
            #print(x_eight.shape) #torch.Size([1, 256, 32, 32])
        else:
